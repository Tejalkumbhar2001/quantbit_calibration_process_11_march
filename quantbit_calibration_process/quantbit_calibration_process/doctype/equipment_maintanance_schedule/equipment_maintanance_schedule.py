...
@frappe.whitelist()
def create_maintanance_orders():
	current_date = datetime.today().date()
	child_data = frappe.get_all("Schedule Of Item",filters = {"is_requested":0},fields=["name","parent"],)
	if(child_data):
		for i in child_data:
			# frappe.throw(str(i.parent))
	...
	# 		for i in child_data:
	# 			# frappe.throw(str(i.parent))
	# 			doc = frappe.get_doc('Equipment Maintanance Schedule', i.parent)
	# 			if doc:
	# 				scheduled_date = frappe.utils.data.getdate(i.scheduled_date)
	# 				# frappe.throw(str((scheduled_date - current_date).days))
	...
	# 					doc.discription = "Preventive Scheduling from Equipment Maintanance Schedule"
	# 					doc.insert()
	# 					frappe.db.set_value('Schedule Of Item', name, 'equipment_status', self.equipment_status)
	# 					# doc.save()
